...
public class EstudianteServiceImpl implements IEstudianteService { // Define la clase EstudianteServiceImpl que implementa la interfaz IEstudianteService
    ...
    @Override
    public List<EstudianteDTO> eliminarEstudiante(Long id){
    ...
    @Override
    public Boolean eliminarEstudianteV2(Long id){
        return estudianteRepository.deleteByIdV2(id);
    }

    //para verificar si existe estudiante
    @Override
    public EstudianteDTO verificarExistencia(Long id){
        if(estudianteRepository.verificarExistencia(id) == null){
            return null;
        }else{
            EstudianteDTO elem = convertToDTO(estudianteRepository.verificarExistencia(id));
            return elem;
        }
    ...
    //para actualizar
    @Override
    public Boolean actualizarEstudiante(Long id, String nombre, String apellido, String gmail, LocalDate fechaNaci, String numIns){
        return estudianteRepository.actualizarEstudiante(id,nombre,apellido,gmail,fechaNaci,numIns);
    }
    public void crear(String nombre, String apellido, String gmail, LocalDate fechaNaci, String numIns){
        EstudianteDTO elem = EstudianteDTO.builder() // Usa el patrón builder para crear un EstudianteDTO
                .id(null) // Asigna el ID
                .nombre(nombre) // Asigna el nombre
                .apellido(apellido) // Asigna el apellido
                .email(gmail) // Asigna el email
                .fechaNacimiento(fechaNaci) // Asigna la fecha de nacimiento
                .numeroInscripcion(numIns) // Asigna el número de inscripción
                .build();
        estudianteRepository.save(convertToEntity(elem));
    }
